...
def runLCG(in_command,in_timeout=300, is_pexpect=True):
    ...
    # print 'soph - inside   def runLCG  in  ND280GRID.py '
    ...
    if is_pexpect:
        ...
        # print 'soph - inside  is_pexpect -   def runLCG  in  ND280GRID.py '
        ...
        print datetime.now()
        # soph changing this from 3 to 12
        while tries<12:
            #print ' soph - next line is    Try pexpect  *tries* of  *in_command*  with  *in_timeout*  s timeout '
            ...
            if pi == 1:
                lines  = output
                errors = []
                print ' soph - pexpect outcome = 1  (which should mean it worked!) '
                break 
            if pi == 2:
                print 'ERROR!'
                print '\n'.join(output)
    ...
    ## Don't use pexpect
        ...
        ## Add lcg-* timeouts
        # print 'soph - inside  not is_pexpect -   def runLCG  in  ND280GRID.py '
...
def runFTSMulti(srm,original_filename,copy_filename,isLastFile=False,ftsInt=0):
    #print 'runFTSMulti: srm='+srm+' orig='+original_filename+' copy='+copy_filename+' isLastFile='+str(isLastFile)+' ftsInt='+str(ftsInt)
...
def GetCurrentRawDataFolder(subdet='ND280',det='ND280'):
    ...
    try:
        ...
        else        : return rmNL(path[len(path)-1])
    except:
...
def GetNRawKEKFiles(lfc_dir):
    ...
    ## slashes (//) in path
    try:
        command = 'mysql -u t2kgsc_reader --password=rdneutgsc -h t2kgscdb.triumf.ca -e \"select * from t2kgscND280.DAQ_FILE_ARCHIVE where DS_directory like \'%'+lfc_dir.split('raw')[1].rstrip('/').replace('//','/')+'\'\" | grep nd280 | wc -l'
        print command
    ...
    def __init__(self, fn, check=True):
        ...
        try:
            if 'lfn:' in fn or 'srm:' in fn or 'guid:' in fn:
                ...
                if rtc:
                    raise self.Error('This file is not registered on the LFC and does not exist on the local system ' + fn)
                self.gridfile=''
        except:
...
##             site_name=os.getenv("HOSTNAME")
    ...
    def Register(self,lfn='',srm='srm-t2k.gridpp.rl.ac.uk', timeout=300):
        ...
        """
	## soph
        #print('soph - inside  Register   in  ND280GRID.py ')    

        ## Local files, lcg-cr
        if not self.gridfile:
	   
	    #print('soph -  inside Register in ND280GRID.py -- Local file ' )
            ...
            #print('soph -  inside Register in ND280GRID.py -- Local file - about to copy and register ' )
            ...
            #print(' soph -1 about to do runLCG(command,in_timeout=timeout) insid Register where command is ')
            #print ' soph -1 command - '
            #print ''.join(command)
            ###### 
            lines,errors = runLCG(command,in_timeout=timeout)
            #print(' soph -1 just done (command,in_timeout=timeout) insid Register ')
            #print ' soph -1 lines - '
            #print '\n'.join(errors)
            ...
            #print(' soph -2 about to do runLCG(command) insid Register where command is checking for replica ')
            #print ' soph -2 command - '
            #print '\n'.join(command)
            ####
            lines,errors = runLCG(command)
            #print(' soph -2 just done runLCG(command) insid Register ')
            #print ' soph -2 lines - '
            #print '\n'.join(lines)
            
            if errors:

	        #print('soph - local file - if errors - inside Register in ND280GRID.py  ')
	        #print ' soph - errors - '
	        #print '\n'.join(errors)
	    ...
	    ## soph
	    #print('soph - inside Register in ND280GRID.py -- grid file ' )
...
##                         continue
    ...
    def SyncSRM(self,srm,use_fts=0,sync_pattern='',ftsInt=0):
        ...
        if failures:
            ...
            raise self.Error('SyncSRM: Could not synchronise ' + str(failures) + ' files between ' + self.dir + ' and ' + str(srm))
    ...
    def SyncND280Dir(self,new_dir,srm='',sync_pattern=''):
        ...
        if failures:
            raise self.Error('SyncND280Dir: Could not synchronise ' + str(failures) + ' files between ' + self.dir + ' and ' + str(srm))
...
class ND280JDL:
    ...
    def CreateJDLFile(self,dir=''):
        ...
        try:
            ...
            if not comp_path:
                raise self.Error('Could not get the ND280COMPUTINGROOT environment variable, have you executed the setup.sh?')

...
# Custom job script that runs nd280Highland2/v1r15 + numuDISkaonAnalysis v0r4 over a flat/mini tree
...
mini_filename = os.path.basename(options.input)
if not os.path.exists(mini_filename):
...
## Need to source nd280Highland2/v1r15 setup script before running any local commands
source_cmd  = 'source %s/nd280Highland2v1r15/setup.sh; ' % os.getenv('VO_T2K_ORG_SW_DIR')

## Create an output file name
kaon_filename = mini_filename.replace('_miniv1r11_','_diskaonv0r4_')

## Process the miniv1r11 file,
exe = 'RunNumuDISkaonAnalysis.exe'
command = source_cmd + exe + ' -o ' + kaon_filename + ' ' + mini_filename
rtc = j.RunCommand(command)
if rtc: print 'Unable to process ' + mini_filename + ' with ' + exe; sys.exit(1)
...
output_path = os.path.dirname(options.input).replace('nd280','nd280/contrib/nirkko')
output_file = ND280File(kaon_filename)

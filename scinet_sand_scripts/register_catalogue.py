...
def get_file(run,subrun,conditions,dataTier):
    ...
    inputfilename = (basedir + "/" + str(run) + "/" + str(subrun)
                  + "/oa_nt_beam_" + str(run) + "-*" + str(subrun) + "*" + dataTier + "*dat"
                 )
...
for run in range(condition.run+0,condition.run+40):
    ...
    for subrun in range(0,120):
        ...
        #print filename
AGHR FIX: WRONG PATH!
    #    command = "storeND280FileCatalogue -p t2kwrite -l /grid/t2k.org/nd280/production005/F/mcp/neut/2010-11-air/sand/beamc/" + dataTier + "/ " + str(fullfilename)
        print command
        os.system(command)
